export class Config {
    ...
    constructor(
        ...
        this.tokenKey = env("MEDIA_SHARE__TokenKey").string();
        this.cookieSecret = env("MEDIA_SHARE__CookieSecret").string();
        ...
        function env(key: string) {
            return {
                string(def?: string, fun?: (val: string) => boolean): string {
                    const val = environment[key] !== undefined ? environment[key] : def;
